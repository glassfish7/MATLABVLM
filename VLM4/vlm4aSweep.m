function [CLpot,CLNvortex,CLvortex,CDiPot,CDiNV,CDiTotal,CDiTreffz] = vlm4aSweep(sections,data,bPanels,cPanels,alpha,Uinf,rho,LE)
...
%Treffz Plane Analysis
treffzPlane = 600*b;
fprintf('Treffz Plane Initialized at %f\n',treffzPlane);
gammaTreffz = sum(gammaMatrix);
wiTreffz = zeros(1,length(controlPointsY));
%Find downwash at the wake
for i = 1:length(controlPointsY)
                    wiTreffz(i) = sum(gammaTreffz.*(-1./(panelGeomY(1:end-1)-controlPointsY(i)) + 1./(panelGeomY(2:end)-controlPointsY(i)) -1./(panelGeomY(1:end-1)+controlPointsY(i)) + 1./(panelGeomY(2:end)+controlPointsY(i)) ))/(4*pi);   
end
inducedDragTreffz = (rho/2)*sum(gammaTreffz.*wiTreffz);
CDiTreffz(z) = 2*inducedDragTreffz/(rho*Uinf^2*S);
