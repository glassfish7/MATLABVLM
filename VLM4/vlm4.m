    ...
    for j = 1:cPanels
        ...
        for k = 1:length(controlPointsY)
            for m = 1:cPanels
                   ...
                   VORTEX(-1*(panelQuarterC(m,k+1)-panelQuarterC(m,k)),panelGeomY(k+1)-panelGeomY(k),0,-1*(panelQuarterC(m,k)-controlPointsX(j,i)),panelGeomY(k)-controlPointsY(i),0)...    
                    + VORTEX(400*b - (-1*panelQuarterC(m,k+1)),0,0,-1*(panelQuarterC(m,k+1)-controlPointsX(j,i)),panelGeomY(k+1)-controlPointsY(i),0)...
                    + VORTEX((-1*panelQuarterC(m,k))- 400*b ,0,0,400*b-(-1*controlPointsX(j,i)),panelGeomY(k)-controlPointsY(i),0)  ;
...
function Vz = VortexVxyz(Gx, Gy, Gz, Rx, Ry, Rz)
...
function w = VORTEX(Gx, Gy, Gz, Rx, Ry, Rz)
