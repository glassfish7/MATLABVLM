function [CL] = vlmAlphaSweepSuc(rootC,taper,S,AR,leLambda,N,alpha,Uinf,rho)
...
RHS = -Uinf*sin(alpha(r))*ones(2*N^2,1);
...
for i = 1:length(controlPointsY)
    for j = 1:NLE
       ...
       normalForce(j,i) = rho*Uinf*gamma*K*cos(alpha(r)) + inducedDragi*sin(alpha(r));
    end
end
...
for i = 1:length(controlPointsY)
    for j = 1:NLE
       ...
       inducedDragi = rho*Uinf*gamma*K*sin(alphai);
       suctionNormalForce(j,i) = (normalForce(j,i)*sin(alpha(r))- inducedDragi)/cos(angleLE);
    end
end

vortexLift =(suctionNormalForce+normalForce)*cos(alpha(r));
