...
leLambda= (90-atand((b/2)/rootC))*(pi/180); %LE Sweep angle in degrees/converted to Radians
% leLambda = 85*(pi/180);
...
LiftDistribution = rho*Uinf*gammaMatrix*K;
...
for i = 1:length(rootChordPoints)
    plot([0 -b/2],[rootChordPoints(i) tipChordPoints(i)],'r');
    plot([0 b/2],[rootChordPoints(i) tipChordPoints(i)],'r');
end
 
for i = 1:length(panelGeomY)
    plot([panelGeomY(i) panelGeomY(i)],[panelGeomX(1,i) panelGeomX(end,i)],'r');
end
...
for i = 1:length(controlPointsY)
    for j = 1:N
        ...
        scatter(controlPointsY(i),controlPointsX(j,i),'b','x');
%         text(controlPointsY(i),panelQuarterC(j,i),txt)
        panelCounter = panelCounter +1;
    end
end

title('All panels and control points on the planform(numbered sequentially)');
xlabel('y');
ylabel('-x');
...
%Plot a vorticity contour plot
figure(3)
contourf(bigY,controlPointsX,gammaMatrix,50,'linecolor','none')
colorbar
title('Vorticity contour plot of planform');
xlabel('y');
ylabel('-x');
colormap jet
...
%gamma matrix
spanwiseLift = sum(LiftDistribution);

%Plot spanwise gamma distribution
figure(4)
plot([-b/2 controlPointsY b/2],[0 spanwiseLift 0])
title('Calculated spanwise gamma distribution');
xlabel('y')
ylabel('\Gamma');

% Lift = rho*Uinf*trapz([-b/2 controlPointsY b/2],[0 gammaDistribution 0]);

Lift = sum(spanwiseLift);

CL = 2*Lift/(rho*S*Uinf^2);
