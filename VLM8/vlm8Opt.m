...
dt = .01*ones(1,1); %Time Step Vector
% dt = [.01*ones(1,30) .05*ones(1,100) .1*ones(1,100)];
...
[controlPointsY,controlPointsX,panelQuarterC,panelTQuarterC,panelGeomY,panelGeomX,VcontrolPointsX,S,b,N,K,CAve,geomChord,chord] = geomEngine3(sections,data,bPanels,cPanels,'NoPlot');
...
%                     [u1,v1,w1] = VORTEX(-1*(panelQuarterC(m,k+1)-panelQuarterC(m,k)),panelGeomY(k+1)-panelGeomY(k),0,-1*(panelQuarterC(m,k)-controlPointsX(j,i)),panelGeomY(k)-controlPointsY(i),0);
                        [u1,v1,w1] = VORTEX(-1*(panelQuarterC(:,2:end)-panelQuarterC(:,1:end-1)),repmat(panelGeomY(2:end)-panelGeomY(1:end-1),cPanels,1),0,-1*(panelQuarterC(:,1:end-1)-controlPointsX(j,i)),repmat(panelGeomY(1:end-1),cPanels,1)-controlPointsY(i),0);
...
%                             -1*(xwHis(f,k)-controlPointsX(j,i)),ywHis(f,k)-controlPointsY(i),zwHis(f,k)); 
                        ...
                        [u2,v2,w2] = VORTEX(-1*(xwHis(f,2:end) - xwHis(f-1,2:end)),ywHis(f,2:end)-ywHis(f-1,2:end),zwHis(f,2:end)-zwHis(f-1,2:end),...
                            -1*(xwHis(f-1,2:end)-controlPointsX(j,i)),ywHis(f-1,2:end)-controlPointsY(i),zwHis(f-1,2:end));

                        [u3,v3,w3] = VORTEX(-1*(xwHis(f-1,1:end-1)- xwHis(f,1:end-1)),ywHis(f-1,1:end-1)-ywHis(f,1:end-1),zwHis(f-1,1:end-1)-zwHis(f,1:end-1),...
                            -1*(xwHis(f,1:end-1)-controlPointsX(j,i)),ywHis(f,1:end-1)-controlPointsY(i),zwHis(f,1:end-1));  
%                         u2=Tfac*u2 + (cPanels-1)*u2; v2=Tfac*v2 + (cPanels-1)*v2; w2=Tfac*w2 + (cPanels-1)*w2; 
%                         u3=Tfac*u3 + (cPanels-1)*u3; v3=Tfac*v3 + (cPanels-1)*v3; w3=Tfac*w3 + (cPanels-1)*w3; 
                      
                        u2 = repmat(u2,cPanels,1);v2 = repmat(v2,cPanels,1);w2 = repmat(w2,cPanels,1);
                        u3 = repmat(u3,cPanels,1);v3 = repmat(v3,cPanels,1);w3 = repmat(w3,cPanels,1);
                        u2(1,:)=Tfac*u2(1,:); v2(1,:)=Tfac*v2(1,:); w2(1,:)=Tfac*w2(1,:); 
                        u3(1,:)=Tfac*u3(1,:); v3(1,:)=Tfac*v3(1,:); w3(1,:)=Tfac*w3(1,:); 
                       ...
                       if panelCount > N/2
                            vorticityWake(panelCount,:) = sum(w2(:)'+w3(:)')*cos(phi)*cos(atan(camberMat(j,i))) -sum(u2(:)'+u3(:)')*sin(atan(camberMat(j,i)))*cos(phi) - sum(v2(:)'+v3(:)')*sin(phi);
                       else
                            vorticityWake(panelCount,:) = sum(w2(:)'+w3(:)')*cos(phi)*cos(atan(camberMat(j,i))) - sum(u2(:)'+u3(:)')*sin(atan(camberMat(j,i)))*cos(phi) - sum(v2(:)'+v3(:)')*sin(-phi);
                       end
                    end
                    ...
                    %Vortex 7: wake to TE
                    if f ~= 1
                        u67 = zeros(cPanels,bPanels*2);
                        v67 = zeros(cPanels,bPanels*2);
                        w67 = zeros(cPanels,bPanels*2);                     
                        [u6s,v6s,w6s] = VORTEX(-1*(xLHis(f,2:end) - xLHis(f-1,2:end)),yLHis(f,2:end)-yLHis(f-1,2:end),zLHis(f,2:end)-zLHis(f-1,2:end),...
                            -1*(xLHis(f-1,2:end)-controlPointsX(j,i)),yLHis(f-1,2:end)-controlPointsY(i),zLHis(f-1,2:end));

                        [u7s,v7s,w7s] = VORTEX(-1*(xLHis(f-1,1:end-1)- xLHis(f,1:end-1)),yLHis(f-1,1:end-1)-yLHis(f,1:end-1),zLHis(f-1,1:end-1)-zLHis(f,1:end-1),...
                            -1*(xLHis(f,1:end-1)-controlPointsX(j,i)),yLHis(f,1:end-1)-controlPointsY(i),zLHis(f,1:end-1)); 
                        u67(1,:) = u7s + u6s;
                        v67(1,:) = v7s + v6s;
                        w67(1,:) = w7s + w6s;                      
                       ...
                       if panelCount > N/2
                            vorticitySep(panelCount,:) =  Lfac*w67(:)*cos(phi)*cos(atan(camberMat(j,i))) - Lfac*u67(:)*sin(atan(camberMat(j,i)))*cos(phi) - Lfac*v67(:)*sin(phi);
                       else
                            vorticitySep(panelCount,:) =  Lfac*w67(:)*cos(phi)*cos(atan(camberMat(j,i))) - Lfac*u67(:)*sin(atan(camberMat(j,i)))*cos(phi) - Lfac*v67(:)*sin(-phi);
                       end
                    end                    
                    ...
                    u67inf = zeros(cPanels,bPanels*2);
                    v67inf = zeros(cPanels,bPanels*2);
                    w67inf = zeros(cPanels,bPanels*2);                        
                    [u2inf,v2inf,w2inf] = VORTEX(wakeT*b - (-1*xwHis(f,2:end)),tan(beta)*(wakeT*b - (-1*xwHis(f,2:end))),tan(alpha)*(wakeT*b - (-1*xwHis(f,2:end))),...
                        -1*(xwHis(f,2:end)-controlPointsX(j,i)),ywHis(f,2:end)-controlPointsY(i),zwHis(f,2:end));

                    [u3inf,v3inf,w3inf] = VORTEX((-1*xwHis(f,1:end-1))- wakeT*b ,tan(beta)*((-1*xwHis(f,1:end-1))- wakeT*b),tan(alpha)*((-1*xwHis(f,1:end-1))- wakeT*b),...
                        wakeT*b-(-1*controlPointsX(j,i)),ywHis(f,1:end-1)+tan(beta)*(wakeT*b - (-1*xwHis(f,1:end-1)))-controlPointsY(i),zwHis(f,1:end-1)+tan(alpha)*(wakeT*b-(-1*xwHis(f,1:end-1))));

                    [u6infs,v6infs,w6infs] = VORTEX(wakeT*b - (-1*xLHis(f,2:end)),tan(beta)*(wakeT*b - (-1*xLHis(f,2:end))),tan(alpha)*(wakeT*b - (-1*xLHis(f,2:end))),...
                        -1*(xLHis(f,2:end)-controlPointsX(j,i)),yLHis(f,2:end)-controlPointsY(i),zLHis(f,2:end));

                    [u7infs,v7infs,w7infs] = VORTEX((-1*xLHis(f,1:end-1))- wakeT*b ,tan(beta)*((-1*xLHis(f,1:end-1))- wakeT*b),tan(alpha)*((-1*xLHis(f,1:end-1))- wakeT*b),...
                        wakeT*b-(-1*controlPointsX(j,i)),yLHis(f,1:end-1)+tan(beta)*(wakeT*b - (-1*xLHis(f,1:end-1)))-controlPointsY(i),zLHis(f,1:end-1)+tan(alpha)*(wakeT*b-(-1*xLHis(f,1:end-1))));

%                     u2inf=(cPanels-1)*u2inf+Tfac*u2inf; v2inf=(cPanels-1)*v2inf+Tfac*v2inf; w2inf=(cPanels-1)*w2inf+Tfac*w2inf;
%                     u3inf =(cPanels-1)*u3inf+Tfac*u3inf; v3inf=(cPanels-1)*v3inf+Tfac*v3inf; w3inf=(cPanels-1)*w3inf+Tfac*w3inf;
%                     u6inf=Lfac*u6inf; v6inf=Lfac*v6inf; w6inf=Lfac*w6inf; u7inf =Lfac*u7inf; v7inf=Lfac*v7inf; w7inf=Lfac*w7inf;

                    u2inf = repmat(u2inf,cPanels,1);v2inf = repmat(v2inf,cPanels,1);w2inf = repmat(w2inf,cPanels,1);
                    u3inf = repmat(u3inf,cPanels,1);v3inf = repmat(v3inf,cPanels,1);w3inf = repmat(w3inf,cPanels,1);
                    u2inf(1,:)=Tfac*u2inf(1,:); v2inf(1,:)=Tfac*v2inf(1,:); w2inf(1,:)=Tfac*w2inf(1,:); 
                    u3inf(1,:)=Tfac*u3inf(1,:); v3inf(1,:)=Tfac*v3inf(1,:); w3inf(1,:)=Tfac*w3inf(1,:); 
                    
                    u67inf(1,:) = u7infs + u6infs;
                    v67inf(1,:) = v7infs + v6infs;
                    w67inf(1,:) = w7infs + w6infs; 
                    
                    %Trailing Vorticies on the wing
                    %Vortex 4: quarter chord to TE
                    %Vortex 5: TE to quarter chord

                    [u4,v4,w4] = VORTEX((-1*repmat(panelGeomX(end,2:end),cPanels,1) - (-1*panelQuarterC(:,2:end))),0,0,-1*(panelQuarterC(:,2:end)-controlPointsX(j,i)),repmat(panelGeomY(2:end),cPanels,1)-controlPointsY(i),0);
                    [u5,v5,w5] = VORTEX((-1*panelQuarterC(:,1:end-1))- (-1*repmat(panelGeomX(end,1:end-1),cPanels,1)),0,0,-1*(repmat(panelGeomX(end,1:end-1),cPanels,1)-controlPointsX(j,i)),repmat(panelGeomY(1:end-1),cPanels,1)-controlPointsY(i),0);

                    [u4L,v4L,w4L] = VORTEX((-1*repmat(panelGeomX(1,2:end),cPanels,1) - (-1*panelQuarterC(:,2:end))),0,0,-1*(panelQuarterC(:,2:end)-controlPointsX(j,i)),repmat(panelGeomY(2:end),cPanels,1)-controlPointsY(i),0);
                    [u5L,v5L,w5L] = VORTEX((-1*panelQuarterC(:,1:end-1))- (-1*repmat(panelGeomX(1,1:end-1),cPanels,1)),0,0,-1*(repmat(panelGeomX(1,1:end-1),cPanels,1)-controlPointsX(j,i)),repmat(panelGeomY(1:end-1),cPanels,1)-controlPointsY(i),0);                        

                    u4(1,:) = u4(1,:)*Tfac;v4(1,:) = v4(1,:)*Tfac;w4(1,:) = w5(1,:)*Tfac;
                    u5(1,:) = u5(1,:)*Tfac;v5(1,:) = v5(1,:)*Tfac;w4(1,:) = w5(1,:)*Tfac;

                    u4L(1,:) = u4L(1,:)*Lfac;v4L(1,:) = v4L(1,:)*Lfac;w4L(1,:) = w4L(1,:)*Lfac;
                    u5L(1,:) = u5L(1,:)*Lfac;v5L(1,:) = v5L(1,:)*Lfac;w5L(1,:) = w5L(1,:)*Lfac;
                    u4L(2:end,:) = 0; v4L(2:end,:) = 0; w4L(2:end,:) = 0;
                    u5L(2:end,:) = 0; v5L(2:end,:) = 0; w5L(2:end,:) = 0;

                    %Sum contributions from each segment in the u,v,w
                    %directions. Excluding the tracked wake.
                    u = u1(:)' + u4(:)' + u4L(:)' + u5(:)' + u5L(:)' + u2inf(:)' + u3inf(:)' + u67inf(:)';
                    v = v1(:)' + v4(:)' + v4L(:)' + v5(:)' + v5L(:)' + v2inf(:)' + v3inf(:)' + v67inf(:)'; 
                    w = w1(:)' + w4(:)' + w4L(:)' + w5(:)' + w5L(:)' + w2inf(:)' + w3inf(:)' + w67inf(:)';
                    
                    %Apply effects of camber and dihedral to contributions
                    %before assigning to vorticity matrix. Note
                    %asymmetircal dihedral angle on each wing
                   if panelCount > N/2
                    vorticityMatrix(panelCount,:) = vorticityWake(panelCount,:) + vorticitySep(panelCount,:) + w*cos(phi)*cos(atan(camberMat(j,i))) - u*sin(atan(camberMat(j,i)))*cos(phi) - v*sin(phi);
                   else
                    vorticityMatrix(panelCount,:) = vorticityWake(panelCount,:) + vorticitySep(panelCount,:) + w*cos(phi)*cos(atan(camberMat(j,i))) - u*sin(atan(camberMat(j,i)))*cos(phi) - v*sin(-phi);
                   end
