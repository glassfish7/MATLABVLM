...
dt = .01*ones(1,4); %Time Step Vector
% dt = [.01*ones(1,30) .05*ones(1,100) .1*ones(1,100)];
wRelaxT = 1; %Factor of Wake Relaxation
wRelaxS = 1;
vS = 0; %Leading edge separation vorticity fraction
Lfac = 1-vS;
...
for k = 1:length(controlPointsY)
    for m = 1:cPanels
                ...
                [u1,v1,w1] = VORTEX(-1*(panelQuarterC(m,k+1)-panelQuarterC(m,k)),panelGeomY(k+1)-panelGeomY(k),0,-1*(panelQuarterC(m,k)-xw),panelGeomY(k)-yw,-zw);

                %Trailing Vortex Contributions. Recalculated on each
                %time step
                u2tot = zeros(1,length(panelGeomY));
                u3tot = zeros(1,length(panelGeomY));
                u6tot = zeros(1,length(panelGeomY));
                u7tot = zeros(1,length(panelGeomY));
                v2tot = zeros(1,length(panelGeomY));
                v3tot = zeros(1,length(panelGeomY));
                v6tot = zeros(1,length(panelGeomY));
                v7tot = zeros(1,length(panelGeomY));
                w2tot = zeros(1,length(panelGeomY));
                w3tot = zeros(1,length(panelGeomY));
                w6tot = zeros(1,length(panelGeomY));
                w7tot = zeros(1,length(panelGeomY));
               ...
               if m == 1
                ...
                for f = 1:timeStep
                    if f ~= 1
                        ...
                        %Vortex 3: wake to TE 
                        [u2,v2,w2] = VORTEX(-1*(xwHis(f,k+1) - xwHis(f-1,k+1)),ywHis(f,k+1)-ywHis(f-1,k+1),zwHis(f,k+1)-zwHis(f-1,k+1),...
                            -1*(xwHis(f-1,k+1)-xw),ywHis(f-1,k+1)- yw,zwHis(f-1,k+1)-zw);

                        [u3,v3,w3] = VORTEX(-1*(xwHis(f-1,k)- xwHis(f,k)),ywHis(f-1,k)-ywHis(f,k),zwHis(f-1,k)-zwHis(f,k),...
                            -1*(xwHis(f,k)-xw),ywHis(f,k)- yw,zwHis(f,k)-zw);

                        u2 =u2*Tfac; v2 =v2*Tfac; w2=w2*Tfac; u3=u3*Tfac; v3=v3*Tfac; w3=w3*Tfac;

                        if m == 1
                            [u6,v6,w6] = VORTEX(-1*(xLHis(f,k+1) - xLHis(f-1,k+1)),yLHis(f,k+1)-yLHis(f-1,k+1),zLHis(f,k+1)-zLHis(f-1,k+1),...
                                -1*(xLHis(f-1,k+1)-xw),yLHis(f-1,k+1)- yw,zLHis(f-1,k+1)-zw);

                            [u7,v7,w7] = VORTEX(-1*(xLHis(f-1,k)- xLHis(f,k)),yLHis(f-1,k)-yLHis(f,k),zLHis(f-1,k)-zLHis(f,k),...
                                -1*(xLHis(f,k)-xw),yLHis(f,k)- yw,zLHis(f,k)-zw);  
                    ...
                    if f == timeStep
                        %Infinite Addon from last known wake points
                        [u2inf,v2inf,w2inf] = VORTEX(wakeT*b - (-1*xwHis(f,k+1)),tan(beta)*(wakeT*b - (-1*xwHis(f,k+1))),tan(alpha)*(wakeT*b - (-1*xwHis(f,k+1))),...
                            -1*(xwHis(f,k+1)-xw),ywHis(f,k+1)-yw,zwHis(f,k+1)-zw);

                        [u3inf,v3inf,w3inf] = VORTEX((-1*xwHis(f,k))- wakeT*b ,tan(beta)*((-1*xwHis(f,k))- wakeT*b),tan(alpha)*((-1*xwHis(f,k))- wakeT*b),...
                            wakeT*b-(-1*xw),ywHis(f,k)+tan(beta)*(wakeT*b-(-1*xwHis(f,k)))-yw,zwHis(f,k)+tan(alpha)*(wakeT*b-(-1*xwHis(f,k)))-zw);
                        if m=='null'
                            [u6inf,v6inf,w6inf] = VORTEX(wakeT*b - (-1*xLHis(f,k+1)),tan(beta)*(wakeT*b - (-1*xLHis(f,k+1))),tan(alpha)*(wakeT*b - (-1*xLHis(f,k+1))),...
                                -1*(xLHis(f,k+1)-xw),yLHis(f,k+1)-yw,zLHis(f,k+1)-zw);

                            [u7inf,v7inf,w7inf] = VORTEX((-1*xLHis(f,k))- wakeT*b ,tan(beta)*((-1*xLHis(f,k))- wakeT*b),tan(alpha)*((-1*xLHis(f,k))- wakeT*b),...
                                wakeT*b-(-1*xw),yLHis(f,k)+tan(beta)*(wakeT*b-(-1*xLHis(f,k)))-yw,zLHis(f,k)+tan(alpha)*(wakeT*b-(-1*xLHis(f,k)))-zw);
                        else
                            u6inf =zeros(1,length(panelGeomY));v6inf=zeros(1,length(panelGeomY));w6inf=zeros(1,length(panelGeomY));
                            u7inf =zeros(1,length(panelGeomY));v7inf=zeros(1,length(panelGeomY));w7inf=zeros(1,length(panelGeomY));
                        end
                        u2inf=Tfac*u2inf; v2inf=Tfac*v2inf; w2inf=Tfac*w2inf; u3inf =Tfac*u3inf; v3inf=Tfac*v3inf; w3inf=Tfac*w3inf;  
                        u6inf=Lfac*u6inf; v6inf=Lfac*v6inf; w6inf=Lfac*w6inf; u7inf =Lfac*u7inf; v7inf=Lfac*v7inf; w7inf=Lfac*w7inf;  
                ...
                %Vortex 5: TE to quarter chord
                if m == 1
                    [u4,v4,w4] = VORTEX((-1*(panelGeomX(end,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xw),panelGeomY(k+1)-yw,-zw);
                    [u5,v5,w5] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(end,k))),0,0,-1*(panelGeomX(end,k)-xw),panelGeomY(k)-yw,-zw);

                    [u4L,v4L,w4L] = VORTEX((-1*(panelGeomX(1,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xw),panelGeomY(k+1)-yw,-zw);
                    [u5L,v5L,w5L] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(1,k))),0,0,-1*(panelGeomX(1,k)-xw),panelGeomY(k)-yw,-zw);                        
                ...
                uw = uw + wRelaxS*gammaMatrix(m,k)*(u1 + u2tot + u3tot + u4 + u5 + u6tot + u7tot);
                vw = vw + wRelaxS*gammaMatrix(m,k)*(v1 + v2tot + v3tot + v4 + v5 + v6tot + v7tot); 
                ww = ww + wRelaxS*gammaMatrix(m,k)*(w1 + w2tot + w3tot + w4 + w5 + w6tot + w7tot);      
