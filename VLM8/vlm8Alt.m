...
dt = [0.01*ones(1,100) .1*ones(1,50) .5*ones(1,20)]; 
%  dt = .01*ones(1,200);
%Initialize wake
xw = panelGeomX(end,:);
yw = panelGeomY;
...
xLHis = zeros(length(time)+1,length(xw));
yLHis = zeros(length(time)+1,length(yw));
zLHis = zeros(length(time)+1,length(zw));
xLHis(1,:) = xL;
yLHis(1,:) = yL;
zLHis(1,:) = zL;
        ...
        for k = 1:length(controlPointsY)  
                for m = 1:cPanels
                    ...
                    if f ~= 1 && m ==1
                        ...
                        [u8,v8,w8] = VORTEX(-1*(xLHis(f,k+1) - xLHis(f,k)),yLHis(f,k+1)-yLHis(f,k),zLHis(f,k+1)-zLHis(f,k),-1*(xLHis(f,k)-controlPointsX(j,i)),yLHis(f,k)-controlPointsY(i),zLHis(f,k));   
                        %Apply effects of camber and dihedral to contributions
                        %before assigning to vorticity matrix. Note
                        %asymmetircal dihedral angle on each wing
                        %Contribution accumulated over timesteps
                    ...
                    [u3inf,v3inf,w3inf] = VORTEX((-1*xwHis(f,k))- wakeT*b ,tan(beta)*((-1*xwHis(f,k))- wakeT*b),tan(alpha)*((-1*xwHis(f,k))- wakeT*b),...
                        ...
                        u2inf=0;v2inf=0;w2inf=0;u3inf =0;v3inf=0;w3inf=0;
                    end
                    %Trailing Vorticies on the wing
                    %Vortex 4: quarter chord to TE
                    %Vortex 5: TE to quarter chord
                    if m == 1
                        [u4,v4,w4] = VORTEX((-1*(panelGeomX(1,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-controlPointsX(j,i)),panelGeomY(k+1)-controlPointsY(i),0);
                        [u5,v5,w5] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(1,k))),0,0,-1*(panelGeomX(1,k)-controlPointsX(j,i)),panelGeomY(k)-controlPointsY(i),0); 
                    else
...
for i = 1:length(panelGeomY) 
    ...
    for k = 1:length(controlPointsY)
        for m = 1:cPanels
                    ...
                    for f = 1:timeStep
                        if f ~= 1
                            ...
                            if m == 1
                                ...
                                [u8,v8,w8] = VORTEX(-1*(xLHis(f,k+1) - xLHis(f,k)),yLHis(f,k+1)-yLHis(f,k),zLHis(f,k+1)-zLHis(f,k),-1*(xLHis(f,k)-xw(i)),yLHis(f,k)-yw(i),zLHis(f,k)-zw(i));  
                            else
                                u6 =0; v6 =0; w6 =0; u7=0; v7=0; w7=0;u8=0;v8=0;w8=0;
                            end
                            u2tot = u2tot + u2;
                            u3tot = u3tot + u3;
                            u6tot = u6tot + u6;
                    ...
                    %Vortex 5: TE to quarter chord
                    if m == 1
                        [u4,v4,w4] = VORTEX((-1*(panelGeomX(1,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xw(i)),panelGeomY(k+1)-yw(i),-zw(i));
                        [u5,v5,w5] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(1,k))),0,0,-1*(panelGeomX(1,k)-xw(i)),panelGeomY(k)-yw(i),-zw(i));
                    else
    ...
    %panel 
        for k = 1:length(controlPointsY)
            for m = 1:cPanels
                ...
                for f = 1:timeStep
                    if f ~= 1
                        ...
                        if m == 1
                            ...
                            [u8,v8,w8] = VORTEX(-1*(xLHis(f,k+1) - xLHis(f,k)),yLHis(f,k+1)-yLHis(f,k),zLHis(f,k+1)-zLHis(f,k),-1*(xLHis(f,k)-xL(i)),yLHis(f,k)-yL(i),zLHis(f,k)-zL(i));
                        else
                            u6 =0; v6 =0; w6 =0; u7=0; v7=0; w7=0; u8=0;v8=0;w8=0;
                        end
                            u2tot = u2tot + u2;
                ...
                %Vortex 5: TE to quarter chord
                if m == 1
                    [u4,v4,w4] = VORTEX((-1*(panelGeomX(1,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xL(i)),panelGeomY(k+1)-yL(i),-zL(i));
                    [u5,v5,w5] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(1,k))),0,0,-1*(panelGeomX(1,k)-xL(i)),panelGeomY(k)-yL(i),-zL(i));              
                else
...
drawSim3(controlPointsY,controlPointsX,panelQuarterC,panelGeomY,panelGeomX,alpha,beta,wakeTPlot,xwHis,ywHis,zwHis,xLHis,yLHis,zLHis,time,bPanels,cPanels,MAC,'freeWake','NOanimate');
