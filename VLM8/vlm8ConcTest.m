...
% xL = [panelGeomX(end-1:-1:1)' panelGeomX(2:1:end-1)'];
% yL = [panelGeomY(end-1:-1:1,1)' panelGeomY(2:end-1,end)'];
xL = [panelQuarterCX(end:-1:1,1)' panelQuarterCX(:,1)'];
yL = [panelQuarterC(end:-1:1,1)' panelQuarterC(:,end)'];
zL = zeros(1,length(xL));
% TipPose = length(xL)/2+1/2;
...
for i = 1:length(xL)
    ...
    %                         [u4L,v4L,w4L] = VORTEX((-1*(panelGeomX(1)) - (-1*panelQuarterCX(m,k))),panelGeomY(1,k+1)-panelQuarterC(m,k+1),0,-1*(panelQuarterCX(m,k)-controlPointsX(j,i)),panelQuarterC(m,k+1)-controlPointsY(i),0);
%                     [u5L,v5L,w5L] = VORTEX((-1*panelQuarterCX(m,k))- (-1*(panelGeomX(m))),panelQuarterC(m,k)-panelGeomY(m,k),0,-1*(panelGeomX(m)-xL(i)),panelGeomY(m,k)-yL(i),-zL(i));   
