...
alpha = 3*(pi/180); % Angle of attack in degrees/converted to Radians 
...
dt = [0.001*ones(1,100) .01*ones(1,100) .1*ones(1,100)]; 
%Initialize wake
xw = panelGeomX(end,:);
yw = panelGeomY;
    ...
    for j = 1:cPanels
        ...
        for k = 1:length(controlPointsY)  
                for m = 1:cPanels
                    ...
                    [u5p,v5p,w5p] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(m,k))),0,0,-1*(panelGeomX(m,k)-controlPointsX(j,i)),panelGeomY(k)-controlPointsY(i),0);
                    u5 = 0.5*u5p;
                    v5 = 0.5*v5p;
                    w5 = 0.5*w5p;
                    elseif k == length(controlPointsY)
                    ...
                    [u4p,v4p,w4p] = VORTEX((-1*(panelGeomX(m,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-controlPointsX(j,i)),panelGeomY(k+1)-controlPointsY(i),0);                    
    ...
    for k = 1:length(controlPointsY)
        for m = 1:cPanels
                    ...
                    if k == 1
                        ...
                        [u5p,v5p,w5p] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(end,k))),0,0,-1*(panelGeomX(end,k)-xw(i)),panelGeomY(k)-yw(i),-zw(i));
                        u5 = 0.5*u5p;
                        v5 = 0.5*v5p;
                        w5 = 0.5*w5p;
                    elseif k == length(controlPointsY)
                        [u4q,v4q,w4q] = VORTEX((-1*(panelTQuarterC(m,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xw(i)),panelGeomY(k+1)-yw(i),-zw(i)); 
                        [u4p,v4p,w4p] = VORTEX((-1*(panelGeomX(end,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xw(i)),panelGeomY(k+1)-yw(i),-zw(i));
        ...
        for k = 1:length(controlPointsY)
            for m = 1:cPanels
                ...
                if k == 1
                    ...
                    [u5p,v5p,w5p] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelGeomX(end,k))),0,0,-1*(panelGeomX(end,k)-xwT(j,tip)),panelGeomY(k)-ywT(j,tip),-zwT(j,tip));
                    u5 = 0.5*u5p;
                    v5 = 0.5*v5p;
                    w5 = 0.5*w5p;                        
                elseif k == length(controlPointsY)
                    ...
                    [u4p,v4p,w4p] = VORTEX((-1*(panelGeomX(end,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xwT(j,tip)),panelGeomY(k+1)-ywT(j,tip),-zwT(j,tip));
