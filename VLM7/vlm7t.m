...
dt = .1; %Time Step for wake progresion
maxTime = 1; %Maximum amount of time for wake development
...
alpha = 9*(pi/180); % Angle of attack in degrees/converted to Radians 
...
dt = [0.001*ones(1,100) .01*ones(1,20) .1*ones(1,10)]; 
%Initialize wake
xw = panelGeomX(end,:);
yw = panelGeomY;
zw = zeros(1,length(panelGeomY));
%Initialize wake history 
xwHis = zeros(length(time)+1,length(xw));
ywHis = zeros(length(time)+1,length(yw));
zwHis = zeros(length(time)+1,length(zw));
xwHis(1,:) = xw;
ywHis(1,:) = yw;
zwHis(1,:) = zw;
...
xwT = [panelTQuarterC(:,1),panelTQuarterC(:,end)];
ywT = repmat([panelGeomY(1),panelGeomY(end)],cPanels,1);
zwT = zeros(cPanels,2);
%Initialize wake history 
xwHisT = zeros(cPanels,2,length(time)+1);
ywHisT = zeros(cPanels,2,length(time)+1);
zwHisT = zeros(cPanels,2,length(time)+1);
xwHisT(:,:,1) = xwT;
ywHisT(:,:,1) = ywT;
zwHisT(:,:,1) = zwT;
%Initilize wake velocity field
                ...
                for m = 1:cPanels
                    ...
                    if f ~= 1 
                        if k == 1
                            ...
                            [u3Tip,v3Tip,w3Tip] = VORTEX(-1*(xwHisT(m,1,f-1)- xwHisT(m,1,f)),ywHisT(m,1,f-1)-ywHisT(m,1,f),zwHisT(m,1,f-1)-zwHisT(m,1,f),...
                                -1*(xwHisT(m,1,f)-controlPointsX(j,i)),ywHisT(m,1,f)-controlPointsY(i),zwHisT(m,1,f));     
                            
                            [u3Trail,v3Trail,w3Trail] = VORTEX(-1*(xwHis(f-1,k)- xwHis(f,k)),ywHis(f-1,k)-ywHis(f,k),zwHis(f-1,k)-zwHis(f,k),...
                                -1*(xwHis(f,k)-controlPointsX(j,i)),ywHis(f,k)-controlPointsY(i),zwHis(f,k)); 
                            
                            u3 = 0.5*(u3Tip+u3Trail);
                            v3 = 0.5*(v3Tip+v3Trail);
                            w3 = 0.5*(w3Tip+w3Trail);
                            
                        elseif k == length(controlPointsY)
                            [u2Tip,v2Tip,w2Tip] = VORTEX(-1*(xwHisT(m,2,f) - xwHisT(m,2,f-1)),ywHisT(m,2,f)-ywHisT(m,2,f-1),zwHisT(m,2,f)-zwHisT(m,2,f-1),...
                                -1*(xwHisT(m,2,f-1)-controlPointsX(j,i)),ywHisT(m,2,f-1)-controlPointsY(i),zwHisT(m,2,f-1));
                            
                            [u2Trail,v2Trail,w2Trail] = VORTEX(-1*(xwHis(f,k+1) - xwHis(f-1,k+1)),ywHis(f,k+1)-ywHis(f-1,k+1),zwHis(f,k+1)-zwHis(f-1,k+1),...
                                -1*(xwHis(f-1,k+1)-controlPointsX(j,i)),ywHis(f-1,k+1)-controlPointsY(i),zwHis(f-1,k+1));
                            
                            u2 = 0.5*(u2Tip+u2Trail);
                            v2 = 0.5*(v2Tip+v2Trail);
                            w2 = 0.5*(w2Tip+w2Trail);                            
                    ...
                    [u3infTip,v3infTip,w3infTip] = VORTEX((-1*xwHisT(m,1,f))- wakeT*b ,tan(beta)*((-1*xwHisT(m,1,f))- wakeT*b),tan(alpha)*((-1*xwHisT(m,1,f))- wakeT*b),...
                    ...
                    [u3infTrail,v3infTrail,w3infTrail] = VORTEX((-1*xwHis(f,k))- wakeT*b ,tan(beta)*((-1*xwHis(f,k))- wakeT*b),tan(alpha)*((-1*xwHis(f,k))- wakeT*b),...
                    ...
                    u3inf = 0.5*(u3infTip + u3infTrail);
                    v3inf = 0.5*(v3infTip + v3infTrail);
                    w3inf = 0.5*(w3infTip + w3infTrail);
                    
                    elseif k == length(controlPointsY)
                    [u2infTip,v2infTip,w2infTip] = VORTEX(wakeT*b - (-1*xwHisT(m,2,f)),tan(beta)*(wakeT*b - (-1*xwHisT(m,2,f))),tan(alpha)*(wakeT*b - (-1*xwHisT(m,2,f))),...
                        -1*(xwHisT(m,2,f)-controlPointsX(j,i)),ywHisT(m,2,f)-controlPointsY(i),zwHisT(m,2,f));
                    
                    [u2infTrail,v2infTrail,w2infTrail] = VORTEX(wakeT*b - (-1*xwHis(f,k+1)),tan(beta)*(wakeT*b - (-1*xwHis(f,k+1))),tan(alpha)*(wakeT*b - (-1*xwHis(f,k+1))),...
                        -1*(xwHis(f,k+1)-controlPointsX(j,i)),ywHis(f,k+1)-controlPointsY(i),zwHis(f,k+1));
                    
                    u2inf = 0.5*(u2infTip + u2infTrail);
                    v2inf = 0.5*(v2infTip + v2infTrail);
                    w2inf = 0.5*(w2infTip + w2infTrail);    
                    ...
                    [u5q,v5q,w5q] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelTQuarterC(m,k))),0,0,-1*(panelTQuarterC(m,k)-controlPointsX(j,i)),panelGeomY(k)-controlPointsY(i),0);
                    [u5p,v5p,w5p] = VORTEX((-1*panelTQuarterC(m,k))- (-1*(panelGeomX(m,k))),0,0,-1*(panelGeomX(m,k)-controlPointsX(j,i)),panelGeomY(k)-controlPointsY(i),0);
                    u5 = u5q + 0.5*u5p;
                    v5 = v5q + 0.5*v5p;
                    w5 = w5q + 0.5*w5p;
                    elseif k == length(controlPointsY)
                    [u4q,v4q,w4q] = VORTEX((-1*(panelTQuarterC(m,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-controlPointsX(j,i)),panelGeomY(k+1)-controlPointsY(i),0);
                    [u4p,v4p,w4p] = VORTEX((-1*(panelGeomX(m,k+1)) - (-1*panelTQuarterC(m,k+1))),0,0,-1*(panelTQuarterC(m,k+1)-controlPointsX(j,i)),panelGeomY(k+1)-controlPointsY(i),0);                    
    ...
    for k = 1:length(controlPointsY)
        for m = 1:cPanels
                    ...
                    if k == 1
                        ...
                        [u5q,v5q,w5q] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelTQuarterC(m,k))),0,0,-1*(panelTQuarterC(m,k)-xw(i)),panelGeomY(k)-yw(i),-zw(i)); 
                        [u5p,v5p,w5p] = VORTEX((-1*panelTQuarterC(m,k))- (-1*(panelGeomX(end,k))),0,0,-1*(panelGeomX(end,k)-xw(i)),panelGeomY(k)-yw(i),-zw(i));
                        u5 = u5q + 0.5*u5p;
                        v5 = v5q + 0.5*v5p;
                        w5 = w5q + 0.5*w5p;
                    elseif k == length(controlPointsY)
                        [u4q,v4q,w4q] = VORTEX((-1*(panelTQuarterC(m,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xw(i)),panelGeomY(k+1)-yw(i),-zw(i)); 
                        [u4p,v4p,w4p] = VORTEX((-1*(panelGeomX(end,k+1)) - (-1*panelTQuarterC(m,k+1))),0,0,-1*(panelTQuarterC(m,k+1)-xw(i)),panelGeomY(k+1)-yw(i),-zw(i));
                    ...
                    uw(i) = uw(i) + wRelaxS*gammaMatrix(m,k)*(u1 + u2tot + u3tot + u4 + u5);
                    vw(i) = vw(i) + wRelaxS*gammaMatrix(m,k)*(v1 + v2tot + v3tot + v4 + v5); 
                    ww(i) = ww(i) + wRelaxS*gammaMatrix(m,k)*(w1 + w2tot + w3tot + w4 + w5);      
        ...
        for k = 1:length(controlPointsY)
            for m = 1:cPanels
                ...
                if k == 1
                    ...
                    [u5q,v5q,w5q] = VORTEX((-1*panelQuarterC(m,k))- (-1*(panelTQuarterC(m,k))),0,0,-1*(panelTQuarterC(m,k)-xwT(j,tip)),panelGeomY(k)-ywT(j,tip),-zwT(j,tip));
                    [u5p,v5p,w5p] = VORTEX((-1*panelTQuarterC(m,k))- (-1*(panelGeomX(end,k))),0,0,-1*(panelGeomX(end,k)-xwT(j,tip)),panelGeomY(k)-ywT(j,tip),-zwT(j,tip));
                    u5 = u5q + 0.5*u5p;
                    v5 = v5q + 0.5*v5p;
                    w5 = w5q + 0.5*w5p;                        
                elseif k == length(controlPointsY)
                    [u4q,v4q,w4q] = VORTEX((-1*(panelTQuarterC(m,k+1)) - (-1*panelQuarterC(m,k+1))),0,0,-1*(panelQuarterC(m,k+1)-xwT(j,tip)),panelGeomY(k+1)-ywT(j,tip),-zwT(j,tip));
                    [u4p,v4p,w4p] = VORTEX((-1*(panelGeomX(end,k+1)) - (-1*panelTQuarterC(m,k+1))),0,0,-1*(panelTQuarterC(m,k+1)-xwT(j,tip)),panelGeomY(k+1)-ywT(j,tip),-zwT(j,tip));
                    ...
                    %Sum contributions from each segment in the u,v,w
                    %directions. Include factor of relaxation here
                    uwT(j,tip) = uwT(j,tip) + wRelaxT*gammaMatrix(m,k)*(u1 + u2tot + u3tot + u4 + u5);
                    vwT(j,tip) = vwT(j,tip) + wRelaxT*gammaMatrix(m,k)*(v1 + v2tot + v3tot + v4 + v5); 
                    wwT(j,tip) = wwT(j,tip) + wRelaxT*gammaMatrix(m,k)*(w1 + w2tot + w3tot + w4 + w5);
...
xwTK1 = -dt(timeStep)*(uwT + Uinf*cos(alpha)*cos(beta));
ywTK1 = dt(timeStep)*(vwT + Uinf*cos(alpha)*sin(beta));
zwTK1 = dt(timeStep)*(wwT + Uinf*sin(alpha));
