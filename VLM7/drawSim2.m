function [] = drawSim2(mainControlPointsY,mainControlPointsX,mainQuarterC,mainPanelGeomY,mainPanelGeomX,alpha,beta,wakeT,xw,yw,zw,xwT,ywT,zwT,time,bPanels,cPanels,MAC,mode,anim)
...
for k = 1:length(mainControlPointsY)
    for j = 1:length(mainQuarterC(:,bPanels+1))
        plot([mainPanelGeomY(k) mainPanelGeomY(k)],[mainQuarterC(j,k) mainPanelGeomX(end,k)],'b')
        plot([mainPanelGeomY(k+1) mainPanelGeomY(k+1)],[mainQuarterC(j,k+1) mainPanelGeomX(end,k+1)],'b')
...
%            plot3([yw(end,k) yw(end,k)+wakeT*tan(beta)],[xw(end,k) -wakeT],[zw(end,k) zw(end,k)+wakeT*tan(alpha)],'b')
       ...
       for n = 1:cPanels
           for tip = 1:2
            plot3(reshape(ywT(n,tip,:),[],1),reshape(xwT(n,tip,:),[],1),reshape(zwT(n,tip,:),[],1),'m')   
%             plot3([ywT(n,tip,end) ywT(n,tip,end)+wakeT*tan(beta)],[xwT(n,tip,end) -wakeT],[zwT(n,tip,end) zwT(n,tip,end)+wakeT*tan(alpha)],'m')  
           end
       end
end
