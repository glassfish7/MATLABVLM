...
dt = (0.041*(rootC)/Uinf)*ones(1,15); %Time Step Vector
dtTraj = (0.010*(rootC)/Uinf).*ones(1,80);
vS = 0.5;
Lfac = vS;
Tfac = 1-vS;
...
xwHisNew = zeros(length(dtTraj)+1,length(xw));
ywHisNew = zeros(length(dtTraj)+1,length(yw));
zwHisNew = zeros(length(dtTraj)+1,length(zw));
xwHisNew(1,:) = xw;
ywHisNew(1,:) = yw;
zwHisNew(1,:) = zw;
...
xLHisNew = zeros(length(dtTraj)+1,length(xw));
yLHisNew = zeros(length(dtTraj)+1,length(yw));
zLHisNew = zeros(length(dtTraj)+1,length(zw));
xLHisNew(1,:) = xL;
yLHisNew(1,:) = yL;
zLHisNew(1,:) = zL;
...
for timeStep = 1:length(dt)
                ...
                for f = 2:length(dtTraj)
                   if timeStep ~= 1
                    ...
                    if f == length(dtTraj)
                        ...
                        [u3inf,v3inf,w3inf] = VORTEX((-1*xwHis(f,k))- wakeT*b ,tan(beta)*((-1*xwHis(f,k))- wakeT*b),tan(alpha/2)*((-1*xwHis(f,k))- wakeT*b),...
                            wakeT*b-(-1*xw(i)),ywHis(f,k)+tan(beta)*(wakeT*b-(-1*xwHis(f,k)))-yw(i),zwHis(f,k)+tan(alpha/2)*(wakeT*b-(-1*xwHis(f,k)))-zw(i));
                        if m==1
                            [u6inf,v6inf,w6inf] = VORTEX(wakeT*b - (-1*xLHis(f,k+1)),tan(beta)*(wakeT*b - (-1*xLHis(f,k+1))),tan(alpha/2)*(wakeT*b - (-1*xLHis(f,k+1))),...
                                -1*(xLHis(f,k+1)-xw(i)),yLHis(f,k+1)-yw(i),zLHis(f,k+1)-zw(i));

                            [u7inf,v7inf,w7inf] = VORTEX((-1*xLHis(f,k))- wakeT*b ,tan(beta)*((-1*xLHis(f,k))- wakeT*b),tan(alpha/2)*((-1*xLHis(f,k))- wakeT*b),...
                                wakeT*b-(-1*xw(i)),yLHis(f,k)+tan(beta)*(wakeT*b-(-1*xLHis(f,k)))-yw(i),zLHis(f,k)+tan(alpha/2)*(wakeT*b-(-1*xLHis(f,k)))-zw(i));
                             u2inf=Tfac*u2inf; v2inf=Tfac*v2inf; w2inf=Tfac*w2inf; u3inf =Tfac*u3inf; v3inf=Tfac*v3inf; w3inf=Tfac*w3inf;  
                             u6inf=Lfac*u6inf; v6inf=Lfac*v6inf; w6inf=Lfac*w6inf; u7inf =Lfac*u7inf; v7inf=Lfac*v7inf; w7inf=Lfac*w7inf;
                        else
                            u6inf =0;v6inf=0;w6inf=0;u7inf =0;v7inf=0;w7inf=0;
            ...
            for m = 1:cPanels
                ...
                for f = 2:length(dtTraj)
                    ...
                    if f ==  length(dtTraj)
                        ...
                        [u3inf,v3inf,w3inf] = VORTEX((-1*xwHis(f,k))- wakeT*b ,tan(beta)*((-1*xwHis(f,k))- wakeT*b),tan(alpha/2)*((-1*xwHis(f,k))- wakeT*b),...
                            wakeT*b-(-1*xL(i)),ywHis(f,k)+tan(beta)*(wakeT*b-(-1*xwHis(f,k)))-yL(i),zwHis(f,k)+tan(alpha/2)*(wakeT*b-(-1*xwHis(f,k)))-zL(i));
                        if m==1
                            [u6inf,v6inf,w6inf] = VORTEX(wakeT*b - (-1*xLHis(f,k+1)),tan(beta)*(wakeT*b - (-1*xLHis(f,k+1))),tan(alpha/2)*(wakeT*b - (-1*xLHis(f,k+1))),...
                                -1*(xLHis(f,k+1)-xL(i)),yLHis(f,k+1)-yL(i),zLHis(f,k+1)-zL(i));

                            [u7inf,v7inf,w7inf] = VORTEX((-1*xLHis(f,k))- wakeT*b ,tan(beta)*((-1*xLHis(f,k))- wakeT*b),tan(alpha/2)*((-1*xLHis(f,k))- wakeT*b),...
                                wakeT*b-(-1*xL(i)),yLHis(f,k)+tan(beta)*(wakeT*b-(-1*xLHis(f,k)))-yL(i),zLHis(f,k)+tan(alpha/2)*(wakeT*b-(-1*xLHis(f,k)))-zL(i));
                             u2inf=Tfac*u2inf; v2inf=Tfac*v2inf; w2inf=Tfac*w2inf; u3inf =Tfac*u3inf; v3inf=Tfac*v3inf; w3inf=Tfac*w3inf;  
                             u6inf=Lfac*u6inf; v6inf=Lfac*v6inf; w6inf=Lfac*w6inf; u7inf =Lfac*u7inf; v7inf=Lfac*v7inf; w7inf=Lfac*w7inf;
                        else
                            u6inf =0;v6inf=0;w6inf=0;u7inf =0;v7inf=0;w7inf=0;
    ...
    xwK1 = -dtTraj(trajStep)*(uw + Uinf*cos(alpha)*cos(beta));
    ywK1 = dtTraj(trajStep)*(vw + Uinf*cos(alpha)*sin(beta));
    zwK1 = dtTraj(trajStep)*(ww + Uinf*sin(alpha));
    xLK1 = -dtTraj(trajStep)*(uL + Uinf*cos(alpha)*cos(beta));
    yLK1 = dtTraj(trajStep)*(vL + Uinf*cos(alpha)*sin(beta));
    zLK1 = dtTraj(trajStep)*(wL + Uinf*sin(alpha));
%  
%     xwK1 = -dtTraj(trajStep)*(Uinf*cos(alpha)*cos(beta));
%     ywK1 = (vw*(xwK1)/(Uinf*cos(alpha)*cos(beta)+uw));
%     zwK1 = (ww*(xwK1)/(Uinf*cos(alpha)*cos(beta)+uw));
%     xLK1 = -dtTraj(trajStep)*(Uinf*cos(alpha)*cos(beta));
%     yLK1 = (vL*(xLK1)/(Uinf*cos(alpha)*cos(beta)+uL));
%     zLK1 = (wL*(xLK1)/(Uinf*cos(alpha)*cos(beta)+uL));
    ...
    xwHisNew(trajStep+1,:) = xw;
    ywHisNew(trajStep+1,:) = yw;
    zwHisNew(trajStep+1,:) = zw;
    xLHisNew(trajStep+1,:) = xL;
    yLHisNew(trajStep+1,:) = yL;
    zLHisNew(trajStep+1,:) = zL;  
...
drawSim3(controlPointsY,controlPointsX,panelQuarterC,panelGeomY,panelGeomX,alpha,beta,wakeTPlot,xwHisNew,ywHisNew,zwHisNew,xLHisNew,yLHisNew,zLHisNew,dt,bPanels,cPanels,MAC,'freeWake','NOanimate');
